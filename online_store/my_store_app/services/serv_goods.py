...
# ============================Формирование каталогов, фильтры, сортировка============================================================================
...
class CatalogByCategoriesMixin:
    ...
    @classmethod
    def get_full_data(cls, tag: str = '', search: str = '') -> Tuple[QuerySet or Any, List]:
        """
        метод возвращает все товары или товары по тэгу или товары подходящие под запрос из строки поиска
        ...
        tags = cls.get_sellers_and_tags(some_goods=goods, main_tag=tag)
    ...
    def get_data_by_tag(cls, some_tag: str) -> QuerySet:
        ...
        return Product.objects.select_related('product') \
    ...
    @classmethod
    def get_full_data_with_filters(cls, search_query: str or None, search_tag: str or None,filter_data: Dict) -> Tuple:
        """
        ...
        tags = cls.get_sellers_and_tags(goods, search_tag)

        return goods, tags

    @classmethod
    def get_sellers_and_tags(cls, some_goods: QuerySet, main_tag: str = '') -> List:
    ...
    def filtering_by_price(cls, filter_data: dict, some_goods_list: QuerySet) -> List:
        ...
        #goods_with_price = (products, prices)
    ...
    def get_min_price(cls, some_list: List) -> int or float:
        ...
        :param some_list: список товаров в магазах
    ...
    def get_request_params_for_full_catalog(cls, request: HttpRequest) -> Tuple:
        """метод возвращает все необходимые параметры гет-запроса для каталога без учета категории"""
...
def get_categories() -> QuerySet:
    ...
    if not categories:
        categories = ProductCategory.objects.select_related('name').all()
