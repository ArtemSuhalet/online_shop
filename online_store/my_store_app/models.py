...
class Profile(models.Model):
    ...
    username = models.CharField(
        default="-------", max_length=50, verbose_name="username", blank=True, null=True
    )
    ...
    avatar = models.ImageField(
        upload_to="catalog/files/", null=True, validators=[validate_image], default=""
    )
...
class ProductCategory(models.Model):
    ...
    name = models.CharField(max_length=25, null=True, verbose_name=_("category title"))
...
class Order(models.Model):
    ...
    DELIVERY_CHOICES = [("reg", _("Regular")), ("exp", _("Express"))]
    ...
    fio = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=_("name and lastname")
    )
    ...
    phone_valid = RegexValidator(
        ...
        message=" ".join(
            [
                str(_("номер телефона необходимо вводить в формате:")),
                "+999999999",
    ...
    city = models.CharField(
        max_length=25, null=True, blank=True, verbose_name=_("city")
    )
    ...
    payment_error = models.CharField(
        max_length=50, null=True, blank=True, verbose_name=_("payment error")
    )

    ordered = models.DateTimeField(
        null=True, blank=True, verbose_name=_("order placement date")
    )
    braintree_id = models.CharField(
        max_length=150, blank=True, verbose_name=_("transaction id")
    )

    delivery_cost = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name=_("Delivery cost")
    )
    ...
    def total_sum(self) -> Decimal:
        ...
        for product in self.order_products.all():
            total += (
                product.final_price * product.quantity + product.order.delivery_cost
            )
...
class OrderProduct(models.Model):
    ...
    final_price = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name=_("final_price")
    )
...
class ViewedProduct(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="viewed", blank=True, null=True
    )
    session = models.CharField(max_length=100, blank=True)
    product = models.ForeignKey(
        "Product", on_delete=models.CASCADE, related_name="viewed_list"
    )
...
class Payment(models.Model):
    number_valid = RegexValidator(
        ...
        message=" ".join(
            [
                str(_("номер  необходимо вводить в формате:")),
                "999999999",
                str(_("до 8 символов.")),
            ]
        ),
    ...
    number = models.IntegerField(
        default=0, validators=[number_valid], verbose_name="номер счета"
    )
