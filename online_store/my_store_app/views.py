...
class FullCatalogView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["CATALOG_MIN_PRICE"], context["CATALOG_MAX_PRICE"] = (
            settings.CATALOG_MIN_PRICE,
            settings.CATALOG_MAX_PRICE,
        )

        context["CATALOG_MIN_SELECTED_PRICE"], context["CATALOG_MAX_SELECTED_PRICE"] = (
            str.split(self.request.GET.get("price"), ";")
            if self.request.GET.get("price")
            else (settings.CATALOG_MIN_PRICE, settings.CATALOG_MAX_PRICE)
        )
    ...
    def get_queryset(self):
        ...
        min_price, max_price = (
            (Decimal(i) for i in str.split(self.request.GET.get("price"), ";"))
            if self.request.GET.get("price")
            else (
                Decimal(settings.CATALOG_MIN_PRICE),
                Decimal(settings.CATALOG_MAX_PRICE),
            )
        )
        is_exist = True if self.request.GET.get("is_exist") == "on" else False
        is_free_delivery = (
            True if self.request.GET.get("free_delivery") == "on" else False
        )
...
def post(request):
    if request.method == "POST":
        ...
        return render(
            request, "catalog.html", {"sort_form": sort_form, "page_obj": page_obj}
        )
...
def add_viewed(request, product_id):
    ...
    if request.user.is_anonymous:
        ViewedProduct.objects.get_or_create(
            session=request.session.session_key, product=product
        )
    else:
...
def ordersteptwo(request, order_id, delivery_cost=None, **kwargs):
    ...
    if request.method == "POST":
        ...
        return render(
            request, "order_step_two.html", {"form": form, "order_id": order_id}
        )
    else:
        order = Order.objects.get(id=order_id)
        user = request.user
        if user.is_authenticated:
            initial = {"delivery": "reg", "payment": "cash"}
...
class OrderStepThree(View):
    ...
    def get(self, request: HttpRequest, order_id):
        ...
        if user.is_authenticated:
            ...
            return render(
                request, self.template_name, {"form": form, "order_id": order_id}
            )
        else:
            return redirect("login")
...
class OrderStepFour(View):
    ...
    def get(self, request: HttpRequest, order_id):
        ...
        if user.is_authenticated:
            ...
            return render(
                request, self.template_name, {"order": order, "order_id": order_id}
            )
...
class PaymentWithCardView(View):
    ...
    def post(self, request: HttpRequest, order_id):
        ...
        return render(
            request, self.template_name, context={"form": form, "order_id": order_id}
        )
